  ...
  return (
    <main className="min-h-screen bg-zinc-950 text-zinc-100 p-8 font-mono">
      ...
      <p className="text-zinc-400 text-sm mb-6">
        Approve to add to your local reading list and push to the live site when configured.
      </p>
      ...
      {!loading && items.length === 0 && (
        <p className="text-zinc-500">
          Queue is empty. Run <code className="text-zinc-300">activity</code> to sync new articles.
        </p>
      )}
      ...
      {items.length > 0 && (
        <p className="text-xs text-zinc-600 mt-8">
          {items.length} article{items.length !== 1 ? 's' : ''} pending
        </p>
      )}
    </main>
  );
